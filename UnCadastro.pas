...
type
  TfrmCadastros = class(TForm)
    ...
    procedure mitAbreCadClick(Sender: TObject);
  private
...
uses
  unPrincipal, System.SysUtils, UnPesquisa, UnCadFunc;
...
procedure TfrmCadastros.mitAbreCadClick(Sender: TObject);
begin
  if qCad.FieldByName('TIPO').AsString = 'Profissional' then
  begin
    if frmCadfunc <> nil then
      frmCadfunc.qCadFunc.Locate('CODIGO', qCorpoCad.FieldByName('CODIGO').AsInteger,[])
    else
    begin
  ...
  begin
    qCad.Close;
    qCad.SQL.Text := 'SELECT * FROM CADASTRO WHERE CODIGO = 0'+ qCorpoCad.FieldByName('CODIGO').AsString;
    qCad.Open;
    btAnterior.Enabled := True;
    btPrimeiro.Enabled := True;
    btProximo.Enabled := True;
...
begin
  if (qCad.State in [dsEdit, dsInsert]) or (qCorpoCad.IsEmpty) then
    Abort;
end;
